...
void ObjectMgr::LoadMailServerTemplates()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `mail_server_template` has moneyA %u or moneyH %u larger than MAX_MONEY_AMOUNT %u for id %u, skipped.", servMail.moneyA, servMail.moneyH, MAX_MONEY_AMOUNT,
                servMail.id);
            return;
        }
...
{ 
    ...
    if (!result_don)
    {
        TC_LOG_ERROR("server.loading", ">>  Loaded 0 Donate vendors. DB table `store_products` is empty!");
    }
    else
    {
        // it's some magic
    ...
    if (!result_cat)
    {
        TC_LOG_ERROR("server.loading", ">>  Loaded 0 Donate Vendor Categories. DB table `store_categories`` is empty!");
    }   
    else
    {
    ...
    if (!result_level)
    {
        TC_LOG_ERROR("server.loading", ">>  Loaded 0 Cost for levelup. DB table `store_level_prices` with price for levelup is empty!");
    } 
    else
    {
